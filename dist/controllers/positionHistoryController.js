...
exports.getPositionsHistory = void 0;
...
const getPositionsHistory = async (req, res) => {
    ...
    if (from || to) {
        query.timestamp = {};
        if (from)
            query.timestamp.$gte = new Date(String(from));
        if (to)
            query.timestamp.$lte = new Date(String(to));
    }
    const lim = Math.min(parseInt(String(limit || "500"), 10), 1000); // Limiter à 1000 max
    try {
        const positions = await Position_1.default.find(query)
            .sort({ timestamp: -1 })
            .limit(lim)
